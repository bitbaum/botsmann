...
interface QuickCreateFlowProps {
  /** Shown above the progress bar while the reader is still choosing a template. */
  intro?: ReactNode;
  /** Shown under the template picker while the reader is still choosing. */
  outro?: ReactNode;
}

/**
 * The three-step "pick a template → name it → chat" flow.
 *
 * One component behind both /create and /personal — they differ only in the
 * copy around the picker, which the pages pass in.
 */
export function QuickCreateFlow({ intro, outro }: QuickCreateFlowProps) {
